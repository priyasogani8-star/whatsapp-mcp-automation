...
exports.AutomationValidator = void 0;
class AutomationValidator {
     ...
     */
    validateSticker(request) {
        if (!request.chatJid)
            throw new Error('chatJid required');
        if (!request.stickerId)
            throw new Error('stickerId required');
        if (!request.packId)
    ...
    validateEvent(request) {
        ...
        if (!request.endTime)
            throw new Error('endTime required');
    ...
    validateMention(request) {
        ...
        if (!request.mentionType)
            throw new Error('mentionType required');
        const validTypes = ['all', 'role', 'specific', 'admin'];
        if (!validTypes.includes(request.mentionType))
            throw new Error('Invalid mention type');
    }
}
exports.AutomationValidator = AutomationValidator;
//# sourceMappingURL=automationValidator.js.map